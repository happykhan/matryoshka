#!/usr/bin/env python3
"""Generate a compact visual acceptance artifact for the MARA ontology."""
...
def main() -> None:
    ...
    (args.out / "mara-component-vocabulary.svg").write_text(
        to_mara_svg(roots, 10_000, "MARA component vocabulary"),
        encoding="utf-8",
    )
    (args.out / "mara-component-table.svg").write_text(
        to_mara_table_svg(roots, "MARA component vocabulary"),
        encoding="utf-8",
    )
    (args.out / "mara-component-catalog.tsv").write_text(
        catalog_as_tsv(),
        encoding="utf-8",
    )
    (args.out / "mara-component-catalog.json").write_text(
        catalog_as_json(),
        encoding="utf-8",
    )
