"""Regression tests for Sally Partridge's supplied non-Tn1/2/3 units."""
...
from matryoshka.partridge_units import UNIT_MAPS, annotate_partridge_units
from matryoshka.reference_scan import REFERENCES_DIR, blast_available, scan

PARTRIDGE_FASTA = (
    Path(__file__).parent / "test-data" / "partridge-examples" / "other_Tn.fasta"
)
UNIT_REFERENCE = REFERENCES_DIR / "mara_partridge_units.fasta"
UNIT_IDS = {
    "Tn1721_AB366441",
    "Tn1722_AB366441",
    "Tn4401_GU595196",
...
def test_tn4401_map_uses_exact_is_names_and_sally_boundaries():
    parent = _parent("Tn4401", "Tn4401_GU595196", 9_907)
    children = annotate_partridge_units([parent])
...
def test_tn21_map_contains_integron_ends_segments_cassette_and_tni_fragment():
    ...
    svg = to_mara_svg(roots, parent.end, "Tn21 MARA map")
    table = to_mara_table_svg(roots, "Tn21 MARA map")
    assert "5&#x27;-CS" in svg
    ...
    assert "MARA-SP-5CS-GGG" in table
    assert "MARA-SP-Tn402-tni" in table


def test_sally_reference_overrides_legacy_duplicate_at_the_same_locus():
    sally = _parent("Tn1721", "Tn1721_AB366441", 11_128)
    sally.attributes["provenance"] = "Sally_Partridge"
    legacy = _parent("Tn3", "Tn1721", 11_128)
    legacy.name = "Tn1721"
    kept = _suppress_redundant_inference([legacy, sally])
    assert kept == [sally]
...
def test_curated_components_override_duplicate_reference_scans_without_disappearing():
    curated_is = MGEFeature(
        ...
        attributes={"source": "curated_reference", "fid": "MARA-SP-ISKpn6"},
    )
    scanned_is = MGEFeature(
        "IS", "IS1182", "ISKpn6", 8066, 9605, "-",
        attributes={"source": "reference_scan", "fid": "MARA-SP-ISKpn6"},
    )
    curated_tni = MGEFeature(
        "transposon_component", "Tn402", "tni", 12358, 15039, ".",
        attributes={"source": "curated_reference", "fid": "MARA-SP-Tn402-tni"},
    )
    scanned_tni = MGEFeature(
        "transposon_component", "Tn402", "tni fragment", 12362, 15039, "+",
        attributes={"source": "reference_scan", "fid": "MARA-SP-Tn402-tni"},
    )
...
def test_all_five_supplied_units_are_exact_reference_calls(tmp_path: Path):
    records = [
        record
        for record in SeqIO.parse(PARTRIDGE_FASTA, "fasta")
        if record.id in UNIT_IDS
    ]
    query = tmp_path / "sally-units.fasta"
    ...
    children = annotate_partridge_units(hits)
