"""Regression tests for the paper-defined MARA feature vocabulary."""
...
from matryoshka.mara_loci import extract_mara_loci
...
def test_renderer_uses_mara_symbols_for_core_feature_vocabulary():
    ...
    svg = to_mara_svg([tn21], 6500, "MARA vocabulary")
...
def test_renderer_draws_the_extended_component_ontology():
    ...
    svg = to_mara_svg([region], 10_000, "extended MARA vocabulary")
    for label in (
        "MRR",
        "attI1",
        "Pc",
...
def test_complete_isecp1_tpu_gets_a_readable_mara_locus_and_table_type():
    tpu = feature(
        "transposition_unit",
        "ISEcp1-blaCMY-2_TPU",
        10_001,
    ...
    tpu.tsd_length = 5
    loci = extract_mara_loci([tpu], 50_000, flank=2_000)
    assert len(loci) == 1
    assert (loci[0].view_start, loci[0].view_end) == (8_001, 16_479)
    svg = to_mara_svg(loci[0].roots, loci[0].view_length, "ISEcp1 TPU")
    table = to_mara_table_svg(loci[0].roots, "ISEcp1 TPU")
...
def test_broad_region_and_mobile_child_get_views_without_duplicate_integron_view():
    ...
    region.children = [tn1]
    loci = extract_mara_loci([region], 100_000, flank=1_000)
...
def test_sally_supplied_exact_is_names_are_recovered():
    supplied = (
    ...
    hits = scan(
        supplied,
        REFERENCES_DIR / "mara_exact_is.fasta",
        min_identity=98.0,
        min_length=500,
    )
...
@pytest.mark.skipif(not blast_available(), reason="blastn not on PATH")
def test_sally_supplied_complete_isecp1_tpus_are_recovered():
    supplied = (
        Path(__file__).parent
        / "test-data"
        / "partridge-examples"
    ...
    hits = scan(
        supplied,
        REFERENCES_DIR / "mara_isecp1_tpu.fasta",
        min_identity=98.0,
        min_length=1_000,
