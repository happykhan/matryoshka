"""Extract readable MARA viewports around validated mobile-element loci."""
...
VALIDATED_MARA_FAMILIES = TN123_FAMILIES | frozenset({
    "Tn21", "Tn1721", "Tn1722", "Tn4401", "Tn5393", "Tn5403",
})
STRUCTURAL_MARA_TARGETS = frozenset({
    "transposition_unit",
...
def _is_mara_target(feature: MGEFeature) -> bool:
    if feature.element_type in STRUCTURAL_MARA_TARGETS:
        return True
    return (
        feature.element_type == "transposon"
        and (
            feature.family in VALIDATED_MARA_FAMILIES
...
def extract_mara_loci(
    roots: list[MGEFeature],
    seq_len: int,
    flank: int = 5_000,
) -> list[MaraLocus]:
    """Return one clipped hierarchy per validated MARA-scale target locus."""
    candidates = [feature for feature in _walk(roots) if _is_mara_target(feature)]
