"""Render annotations as compact, single-line locus maps.
...
def _fragment_block(feature: MGEFeature, seq_len: int) -> str:
    """locus side-specific jagged ends for a partial feature."""
    x1, x2 = _x(feature.start, seq_len), _x(feature.end, seq_len)
...
def _block_arrow(
    ...
    *,
    ...
    """Block arrow; for an IS, the pointed end denotes IRR."""
    x1, x2 = _x(feature.start, seq_len), _x(feature.end, seq_len)
    width = max(x2 - x1, 4.0)
...
def _legend() -> str:
    """Self-contained key explaining the locus-map notation."""
    top = 164
    row1 = 194
    row2 = 222
    row3 = 250
...
def to_locus_map_svg(
    roots: list[MGEFeature],
    seq_len: int,
    sample_name: str = "",
) -> str:
    """Render features on one line using the configured locus-map grammar."""
    ...
    for feature in major_units:
        ...
        # the established locus position above the feature block.
        parts.append(_text(
            mid,
