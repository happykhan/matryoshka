...
def build_tn123_proof(
    ...
    for seqid, length, roots in records:
        ...
        for parent in _tn123_parents(roots):
            ...
            if not isinstance(requirements, dict):
                requirements = {}
            components = _detected_components(parent)
            ...
            checks = {
                "components_detected_from_sequence": (
                    len(components) >= 6
                    and all(
                        feature.attributes.get("evidence_class") == "sequence_detected"
            ...
            loci.append({
                ...
                "checks": checks,
                "components": [
                    _component_document(component) for component in components
                ],
    ...
    status = "PASS" if total and fail_count == 0 else (
        "NO_TN123_LOCI" if total == 0 else "FAIL"
    )
    return {
...
def proof_matches_tsv(proof: dict[str, Any]) -> str:
    ...
    fields = [
        ...
        "whole_locus_coverage", "variant_status", "structural_status",
...
.pass{{background:#d7f5e5;color:#096b3b}}.fail{{background:#fde0e0;color:#9b1c1c}}
...
<p><b>{summary['status']}</b>: {summary['passed']} of {summary['tn123_loci']} Tn1/Tn2/Tn3 loci passed every component, grammar, match and projection check.</p>
