...
def _validate_unit(family: str, definition: object) -> None:
    ...
    roles = [_validate_unit_component(family, component) for component in components]
    required_counts = {
        str(role): int(count)
...
def _validate_feature_maps(feature_maps: object) -> None:
    ...
    for map_name, parts in feature_maps.items():
        ...
        for part in parts:
            ...
            if missing:
                raise ValueError(
                    f"feature map {map_name} part is missing: {', '.join(sorted(missing))}"
                )
...
def load_unit_definitions() -> dict[str, Any]:
    ...
    for family, definition in units.items():
        _validate_unit(family, definition)

    _validate_feature_maps(document["feature_maps"])
    return document
