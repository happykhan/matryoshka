# Locus annotation specification

This document defines the target for Matryoshka's automated locus-compatible
annotation. The aim is biological and representational parity with published and
expert-reviewed practice, not merely a visual imitation of an example figure.
...
- expert-reviewed Tn1/Tn2/Tn3, ISEcp1 TPU and other-transposon FASTAs and
  worked examples, plus the 1 June 2026 review comments on the Matryoshka
  demonstration.
...
2. Reserve their coordinates before resolving fragments and overlaps. This
   implements expert-reviewed recommendation that a complete feature overrides a
...
4. Annotate partial features only when sufficient diagnostic sequence remains.
   The published thresholds were at least 19 bp from a feature end or 32 bp
...
[`locus-component-inventory.md`](locus-component-inventory.md) and the
machine-readable [`locus_component_catalog.yaml`](../matryoshka/locus_component_catalog.yaml).
The table below remains the concise parity checklist.

| Feature | Required interpretation | Locus-map symbol | Current state |
| --- | --- | --- | --- |
...
- Include a self-contained key on every map so readers do not need prior
...
- pKpQIL, pEK499, pOXA-48a and the R100 worked example when complete source
